	...
	$scope.checkKitchen = function(item){
		...
		for(var i=0; i<$scope.kitchen.length; i++){
			if($scope.kitchen[i].id == item.id){
				...
				if(updateKitchen.quanityNum >= item.quanity){
					updateKitchen.quanityNum -= item.quanity;
					updateKitchen.menuQuanityNum += item.quanity;
					console.log("Asking for less than what is in the Kitchen");
				}else {
					var diff = item.quanity - updateKitchen.quanityNum;
				...
				updateGrocery.quanityNum = item.quanity + groceryQuanity;
				$scope.newItem = updateGrocery;
				$scope.create("groceryList");
			}
		}
